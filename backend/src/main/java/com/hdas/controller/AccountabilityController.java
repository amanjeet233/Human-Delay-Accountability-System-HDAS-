...
import com.hdas.repository.DelegationRepository;
...
public class AccountabilityController {
    ...
    @PreAuthorize("hasRole('HOD')")
    public ResponseEntity<?> getDelegationsByAssignment(@PathVariable UUID assignmentId, HttpServletRequest httpRequest) {
        if (!featureFlagService.isFeatureEnabled("advancedAccountability")) {
        ...
        return ResponseEntity.ok(accountabilityService.getDelegationsByAssignment(assignmentId));
    }
    ...
    public ResponseEntity<?> getDelayDebtScore(
        ...
        DelayDebtScore score = accountabilityService.getDelayDebtScore(userId, roleId);
    ...
    public ResponseEntity<?> calculateDelayDebt(
        ...
        accountabilityService.calculateDelayDebt(userId, roleId);
